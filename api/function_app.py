...
APP_DISPLAY_NAME = os.environ.get("APP_DISPLAY_NAME", "Reliance Email Campaign Generator").strip()
...
def _snovio_list_name(payload: dict, job_id: str) -> str:
    ...
    parts = ["Reliance", template_name]
    if source_file:
        parts.append(source_file.rsplit(".", 1)[0])
    parts.append(date_suffix)
    name = " - ".join(part for part in parts if part)
    return (name or f"Reliance - {job_id[:8]} - {date_suffix}")[:120]
...
def _copilot_app_tools(user: dict, req: func.HttpRequest, durable_client=None, mcp_session=None) -> dict:
    """Owner-scoped app and policy-routed Snov.io tools for Copilot."""
